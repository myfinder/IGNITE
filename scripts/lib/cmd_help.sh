...
cmd_help() {
    ...
    case "$command" in
        start)
            ...
            echo "  --dry-run             Phase 1-5,8のみ実行し、エージェント起動をスキップ"
            echo "  -h, --help            このヘルプを表示"
            echo ""
            echo "ペインレイアウト (fullモード):"
            echo "  pane 0:    Leader ($LEADER_NAME)"
            echo "  pane 1-5:  Sub-Leaders ($(printf '%s' "${SUB_LEADER_NAMES[0]}"; printf ', %s' "${SUB_LEADER_NAMES[@]:1}"))"
        ...
        status)
            ...
            ;;
        attach)
            echo "使用方法: ./scripts/ignite attach [agent_name] [オプション]"
            echo ""
            echo "セッションに接続します"
            echo ""
            echo "引数:"
            echo "  [agent_name]        接続するエージェント名（ヘッドレスモード）"
            echo ""
            echo "オプション:"
            ...
            echo ""
            echo "例:"
            echo "  ./scripts/ignite attach              # セッションに接続（選択式）"
            echo "  ./scripts/ignite attach leader       # 指定エージェントに接続（ヘッドレス）"
        ...
        watcher)
            ...
            ;;
        service)
            echo "使用方法: ./scripts/ignite service <action> [arguments]"
            echo ""
            echo "systemd ユーザーサービスとして IGNITE を管理します"
            echo ""
            echo "アクション:"
            echo "  install              ユニットファイルをインストール"
            echo "  uninstall            ユニットファイルをアンインストール"
            echo "  enable <session>     サービスを有効化（自動起動）"
            echo "  disable <session>    サービスの自動起動を無効化"
            echo "  start <session>      サービスを開始"
            echo "  stop <session>       サービスを停止"
            echo "  restart <session>    サービスを再起動"
            echo "  status [session]     サービスの状態を表示"
            echo "  logs <session>       ジャーナルログを表示"
            echo "  setup-env <session>  環境変数ファイルを対話的に生成"
            echo "  help                 このヘルプを表示"
            echo ""
            echo "オプション:"
            echo "  -y, --yes, --force   確認プロンプトをスキップ (install, setup-env)"
            echo "  --no-follow          ログのリアルタイム追跡を無効化 (logs)"
            echo ""
            echo "初回セットアップ:"
            echo "  1. ignite service install          # ユニットファイル配置"
            echo "  2. ignite service setup-env my-proj # 環境変数設定"
            echo "  3. ignite service enable my-proj   # 自動起動有効化"
            echo "  4. loginctl enable-linger \$(whoami)  # linger有効化"
            echo "  5. ignite service start my-proj    # サービス開始"
            echo ""
            echo "例:"
            echo "  ./scripts/ignite service install"
            echo "  ./scripts/ignite service setup-env my-project"
            echo "  ./scripts/ignite service enable my-project"
            echo "  ./scripts/ignite service start my-project"
            echo "  ./scripts/ignite service status"
            echo "  ./scripts/ignite service logs my-project --no-follow"
            ;;
        "")
