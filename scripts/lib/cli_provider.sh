...
cli_get_env_vars() {
    case "$CLI_PROVIDER" in
        claude)
            ...
            ;;
        opencode)
            # OPENCODE_CONFIG はロール別に各エージェントで設定されるため出力しない
            ;;
    esac
}
...
# =============================================================================
# ヘッドレスモード判定・管理関数群 (opencode serve)
# =============================================================================

# cli_is_headless_mode - ヘッドレスモードかどうかを判定
# opencode → ヘッドレス (0), claude → TUI (1)
cli_is_headless_mode() {
    [[ "${CLI_PROVIDER:-opencode}" == "opencode" ]]
}
