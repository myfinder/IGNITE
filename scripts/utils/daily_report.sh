...
log_info() { echo -e "${BLUE}[REPORT]${NC} $1" >&2; }
log_success() { echo -e "${GREEN}[REPORT]${NC} $1" >&2; }
log_warn() { echo -e "${YELLOW}[REPORT]${NC} $1" >&2; }
log_error() { echo -e "${RED}[REPORT]${NC} $1" >&2; }
...
cmd_comment() {
    ...
    if _gh_api "$repo" issue comment "$issue_num" --repo "$repo" --body "$body" &>/dev/null; then
...
EOF
    ...
    _gh_api "$repo" issue comment "$issue_num" --repo "$repo" --body "$close_comment" &>/dev/null || true
