...
ensure_label() {
    ...
    if github_api_get "$repo" "/repos/$repo/labels/$REPORT_LABEL" &>/dev/null; then
        return 0
    fi

    log_info "ラベル '$REPORT_LABEL' を作成中..."
    local label_json
    label_json=$(jq -n --arg name "$REPORT_LABEL" --arg color "$REPORT_LABEL_COLOR" --arg desc "$REPORT_LABEL_DESCRIPTION" \
...
find_today_issue() {
    ...
    if [[ -n "$cached_num" ]]; then
        ...
        local state
        state=$(github_api_get "$repo" "/repos/$repo/issues/$cached_num" 2>/dev/null | jq -r '.state // empty' || echo "")
        if [[ "$state" == "open" ]]; then
            echo "$cached_num"
            return 0
        fi
    ...
    local issue_num
    local search_query
    search_query="repo:${repo} is:issue is:open label:${REPORT_LABEL} in:title \"${title_prefix}\""
    issue_num=$(github_api_get "$repo" "/search/issues?q=$(python3 -c "import urllib.parse; print(urllib.parse.quote('$search_query'))")&per_page=1" 2>/dev/null | jq -r '.items[0].number // empty' 2>/dev/null || echo "")
...
cmd_create() {
    ...
    local issue_url
    local issue_json
    issue_json=$(jq -n --arg title "$title" --arg body "$body" '{"title": $title, "body": $body, "labels": ["'"$REPORT_LABEL"'"]}')
    ...
    # レスポンスから Issue 番号を抽出
    local issue_num
    issue_num=$(printf '%s' "$issue_response" | jq -r '.number // empty' || true)
...
cmd_update() {
    ...
    local edit_json
    edit_json=$(jq -n --arg body "$body" '{"body": $body}')
    if github_api_request "$repo" "PATCH" "/repos/$repo/issues/$issue_num" "$edit_json" &>/dev/null; then
...
EOF
    ...
    local close_json
    close_json=$(jq -n --arg body "$close_comment" '{"body": $body}')
    github_api_post "$repo" "/repos/${repo}/issues/${issue_num}/comments" "$close_json" &>/dev/null || true
    local close_state_json='{"state": "closed"}'
    if github_api_request "$repo" "PATCH" "/repos/$repo/issues/$issue_num" "$close_state_json" &>/dev/null; then
