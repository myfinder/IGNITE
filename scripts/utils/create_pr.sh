...
main() {
    ...
    local issue_info=$(get_issue_info "$REPO" "$ISSUE_NUMBER")
    ...
    local issue_body=$(echo "$issue_info" | jq -r '.body // ""')
...
*Generated by IGNITE AI Team*"
        ...
        # PR作成
        local pr_url=$(create_pull_request "$REPO" "$pr_title" "$pr_body" "$base_branch" "$branch_name" "$is_draft" "$use_bot")
