...
2. **メッセージ処理**
   ...
   - typeに応じて適切に処理:
     ...
     - `slack_event`: Slack Watcherからのイベント通知（自分宛メンション）
     - `slack_task`: Slack Watcherからのタスクリクエスト（タスクキーワード検出時）
...
6. **完了報告**
   ...
   - ログを出力
   - 処理完了後は待機状態に戻る（次の通知は queue_monitor が通知します。自分からキューをチェックしないでください）
...
### GitHub Task結果出力
- github_task 受信時: 結果は必ず `comment_on_issue.sh` でGitHubコメントとして投稿する
...
1. **受付応答**: まず受付メッセージを投稿
   ...
   ./scripts/utils/post_to_slack.sh --channel {channel_id} --thread-ts {thread_ts} --template acknowledge
   ```
2. **タスク実行**: 意図分類に基づきタスクを直接実行（solo mode なので委任なし）
...
### Slack Task結果出力
- slack_task 受信時: 結果は必ず `post_to_slack.sh` で Slack スレッドに投稿する
- `workspace/` 配下に分析結果・レポートファイルを出力しない
- 一時ファイルは `.ignite/tmp/` に作成し、投稿後に削除する
...
**重要: 初期化完了後は待機してください。新しいメッセージが到着すると queue_monitor が通知します。自発的にキューをポーリングしないでください。**
